...
//   var apiKey = jsonData[0].key;
  ...
  async function syncToInterval(interval) {
	...
	if(interval == '1M' || interval == '1Y'){
		areaSeries.update({ time: moment.unix(thisCurrentTime/1000000000).format('YYYY-MM-DD'), value: thisCurrentPrice });
	}else if(interval == '1D' || interval == '1W'){
		areaSeries.update({time: thisCurrentTime/1000000000, value: thisCurrentPrice});
	}
  }
			...
			data => {
				...
				results.forEach(result => {
					...
					if(chartType == 'month' || chartType == 'year'){
						dayDataResult.push(
							{ time: moment.unix(result.t/1000).format('YYYY-MM-DD'), value: result.o }
						);
					}else{
						dayDataResult.push(
							{ time: result.t/1000, value: result.o }
						);
					}
				});
				// if(chartType == 'month' || chartType == 'year'){
				// 	dayDataResult.pop();
				// 	dayDataResult.push(
				// 		{ time: moment.unix(thisCurrentTime/1000000000).format('YYYY-MM-DD'), value: thisCurrentPrice }
				// 	);
				// 	// console.log(moment.unix(thisCurrentTime/1000000000).format('YYYY-MM-DD HH:mm'));
				// }else{
				// 	dayDataResult.push(
				// 		{ time: thisCurrentTime/1000000000, value: thisCurrentPrice }
				// 	);
				// }
				
				// console.log(thisCurrentTime);
