...
window.addEventListener('DOMContentLoaded', event => {
  ...
  let dayData, weekData, monthData, yearData, thisCurrentPrice;
  let thisCurrentTime = 0;
  getSnapshot(apiKey);
  ...
  function createSimpleSwitcher(items, activeItem, activeItemChangedCallback) {
    ...
    let intervalElements = items.map(function (item) {
      ...
      itemEl.addEventListener('click', function (e) {
        e.preventDefault();
        onItemClicked(item);
        selectedView = item; // setLastBarText(selectedView);
      });
  ...
  let seriesesData = new Map([['1D', dayData], ['1W', weekData], ['1M', monthData], ['1Y', yearData]]);
  ...
  async function syncToInterval(interval) {
    ...
    console.log(data);
    let secLatestPrice = data[0].value;
    console.log(secLatestPrice);
  ...
  chartElement.appendChild(toolTip); // update tooltip
  ...
  chart.subscribeCrosshairMove(function (param) {
    if (param.point === undefined || !param.time || param.point.x < 0 || param.point.x > chartElement.clientWidth || param.point.y < 0 || param.point.y > chartElement.clientHeight) {
      ...
      if (dateTest._i['day']) {
        ...
        var dateStr = moment.unix(param.time).format('MM/DD/YY hh:mm A');
      }

      toolTip.style.display = 'block';
      let price = param.seriesPrices.get(areaSeries);
      toolTip.innerHTML = '<div style="color: #c4212d">Rubius Therapeutics Inc.</div><div style="font-size: 24px; margin: 4px 0px; color: #21384d">$' + (Math.round(100 * price) / 100).toFixed(2) + '</div><div style="color: #21384d">' + dateStr + '</div>';
  ...
  */
  ...
  window.addEventListener('resize', resize); //Get SnapShot of a ticker (RUBY)

  function getSnapshot(apiKey) {
    let currentPrice = 0;
    let todayOpen = 0;
    let todaysChange = 0;
    let todaysChangePercent = 0; //CHECK MARKET STATUS
    //GET CURRENT PRICE/ASK-PRICE

    fetch('https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/RUBY?&apiKey=' + apiKey).then(response => response.json()).then(data => {
      ...
      let tickerTodayChangePercent = tickerSnapshot.todaysChangePerc; // console.log(tickerSnapshot);
      // manipulate the DOM

      thisCurrentPrice = tickerLastQuote["p"];
      thisCurrentTime = tickerLastQuote["t"];
      document.querySelector('.rubius-stock-price_price').innerHTML = "$" + tickerLastQuote["p"].toFixed(2);
  ...
  function getTrade(apiKey, chartType) {
    ...
    return fetch('https://api.polygon.io/v2/aggs/ticker/RUBY/range/' + multiplier + '/' + timeSpan + '/' + day_from + '/' + day_to + '?unadjusted=true&sort=asc&limit=50000&apiKey=' + apiKey).then(response => response.json()).then(data => {
      let results = data.results;
      results.forEach(result => {
        // console.log(moment.unix(result.t/1000).format('YYYY-MM-DD HH:mm'));
        if (chartType == 'month' || chartType == 'year') {
          dayDataResult.push({
            time: moment.unix(result.t / 1000).format('YYYY-MM-DD'),
            value: result.o
          });
        } else {
          dayDataResult.push({
      ...
      if (chartType == 'month' || chartType == 'year') {
        dayDataResult.pop();
        dayDataResult.push({
          time: moment.unix(thisCurrentTime / 1000000000).format('YYYY-MM-DD'),
          value: thisCurrentPrice
        }); // console.log(moment.unix(thisCurrentTime/1000000000).format('YYYY-MM-DD HH:mm'));
      } else {
        dayDataResult.push({
          time: thisCurrentTime / 1000000000,
          value: thisCurrentPrice
        });
